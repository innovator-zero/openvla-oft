...
class RealEnv:
    ...
    def __init__(self, init_node, setup_robots=True):
        ...
        self.puppet_bot_right = InterbotixManipulatorXS(
            robot_model="vx300s", group_name="arm", gripper_name="gripper", robot_name=f"puppet_right", init_node=False
        )
        if setup_robots:
            self.setup_robots()
    ...
    def get_observation(self, t=0):
        ...
        return dm_env.TimeStep(step_type=step_type, reward=self.get_reward(), discount=None, observation=self._get_obs())
    ...
    def reset(self, fake=False):
        ...
        return dm_env.TimeStep(
            step_type=dm_env.StepType.FIRST, reward=self.get_reward(), discount=None, observation=self._get_obs()
        )
    ...
    def step(self, action):
        ...
        time.sleep(DT)
        return dm_env.TimeStep(
            step_type=dm_env.StepType.MID, reward=self.get_reward(), discount=None, observation=self._get_obs()
        )
